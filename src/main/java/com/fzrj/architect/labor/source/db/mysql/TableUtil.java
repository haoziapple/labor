...
/**
 * @className:com.fzrj.architect.labor.utils.TableUtil
 * @description:数据库表工具类
 * @version:v1.0.0
 * @date:2017年6月22日 下午5:14:12
 * @author:WangHao
 */
...
public abstract class TableUtil
{
	...
	public static final List<Table> getTables(Connection conn, String packageName, String[] tableNames) throws Exception
	{
		...
		for (String tableName : tabelNames)
		{
			...
			{
				// ResultSet trs = conn.getMetaData().getColumns(null, "%",
				// tableName,"%");
				...
				if (StringUtils.isBlank(table.getPrimaryKey()))
				{
					throw new NullPointerException(
							"table '" + tableName + "' without primary key, please setting the " + "primary key.");
				}
	...
	public static final List<TableIndex> getTableIndexs(Connection conn, String tableName) throws Exception
	{
		...
		while (it.hasNext())
		{
			...
			for (Map map : vals)
			{
				...
				String carrayName_d = StringUtil
						.upperFirst(PinYinUtil.getFirstSpell(StringUtil.newTableName(carrayName)));
				String carrayName_x = StringUtil
						.lowerFirst(PinYinUtil.getFirstSpell(StringUtil.newTableName(carrayName)));
